...
import com.electronic.armony.domain.Program;
import com.electronic.armony.repository.ProgramRepository;
import com.electronic.armony.service.ProgramService;
import com.electronic.armony.service.dto.ProgramDTO;
import com.electronic.armony.service.mapper.ProgramMapper;
...
import static com.electronic.armony.web.rest.TestUtil.createFormattingConversionService;
...
public class ProgramResourceIntTest {
    ...
    public void getAllProgramsWithEagerRelationshipsIsEnabled() throws Exception {
        ProgramResource programResource = new ProgramResource(programServiceMock);
        when(programServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restProgramMockMvc = MockMvcBuilders.standaloneSetup(programResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();
